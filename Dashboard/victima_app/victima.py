...
while True:
    try:
        ...
        if doc.exists:
            ...
            alerta_activa = data.get('activa', False)
            nivel = "CRITICO" if alerta_activa == True else "NORMAL"
            ...
            capas_mapa = [
                ...
                pdk.Layer("LineLayer", data=[{'start': [a_lon, a_lat], 'end': [t_lon, t_lat]}], get_source_position="start", get_target_position="end", get_color=color_linea, get_width=1, width_min_pixels=4),
                pdk.Layer("TextLayer", data=[{'pos': [mid_lon, mid_lat], 'text': texto_mapa}], get_position="pos", get_text="text", get_size=20, get_color=[15, 23, 42, 255], outline_width=3, outline_color=[255, 255, 255, 255], get_alignment_baseline="'center'", get_text_anchor="'middle'"),
            ...
            with placeholder_info.container():
                if nivel == "CRITICO":
                    ...
                    html_content = f"""<div class="{card_class}">
                        ...
                        <div class='metric-value metric-value-danger'>{int(dist)} <span style='font-size: 1.5rem;'>m</span></div>
                        ...
                        <div style='color: #b91c1c; font-weight: 600; margin-top: 10px;'>Protocolo activo: Vulneración de perímetro detectada. Por favor, póngase a salvo.</div>
                    ...
                    html_content = f"""<div class="{card_class}">
                        <div class="label-title">Estado del Sistema</div>
                        {status_title}
                        <div style="margin-top: 10px; margin-bottom: 25px; font-size: 1.1rem; color: #059669; font-weight: 600; border-left: 4px solid #059669; padding-left: 10px;">
                            El perímetro está asegurado.<br>No hay amenazas detectadas.
                        </div>
                        <div class="label-title" style="margin-top: 20px;">Punto Protegido ({icono_label})</div>
                        <div style="font-size: 1.1rem; color: #334155; font-weight: 500;">{nombre_objetivo}</div>
                        <div class="timestamp">Última lectura del radar: {datetime.now().strftime('%H:%M:%S')}</div>
