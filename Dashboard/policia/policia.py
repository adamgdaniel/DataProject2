...
# URL Base de tu API (Configúrala en tu .env)
...
st.markdown("""
    ...
    div.stButton > button { background-color: #003366; color: white; border: none; font-weight: bold; width: 100%; }
...
# ==========================================
# 2. CONEXIONES A BASES DE DATOS (FIRESTORE Y API)
# ==========================================
...
def obtener_datos_api():
    """Obtiene todos los datos maestros desde la API (BigQuery)"""
    try:
        ...
        # 3. Safe Places (Mapeamos 'place_name' a 'nombre' para compatibilidad visual)
        ...
        # 4. Relaciones (Cruzamos para la tabla del Dashboard)
...
# Lookup para Lugares (Radar)
place_lookup = {}
if not df_safe_places.empty:
    for _, row in df_safe_places.iterrows():
        place_lookup[row['id_place']] = row['nombre']
...
# ------------------------------------------------------------------
# TAB 1: MONITORIZACIÓN GPS (SINCRONIZADA)
# ------------------------------------------------------------------
with tab_monitor:
    ...
    if activar_streaming:
        while activar_streaming:
            try:
                docs = st.session_state.db_fs.collection("alertas").stream()
                ...
                for doc in docs:
                    ...
                    feed_alertas.append({'agr': nombres['agr'], 'obj': nombre_obj, 'icono': icono, 'dist': dist, 'nivel': nivel})
                ...
                with placeholder_feed.container():
                    ...
                    for a in sorted(feed_alertas, key=lambda x: (x['nivel']!='CRITICO', x['dist'])):
                        ...
                        f_html += f"<div class='{cl}'><b>{a['agr']}</b><br><small>{a['icono']} {a['obj']}</small><div class='{'dist-critical' if a['nivel']=='CRITICO' else 'dist-safe'}'>{int(a['dist'])} m</div></div>"
                    st.markdown(f_html + "</div>", unsafe_allow_html=True)
                time.sleep(2)
            except Exception as e:
...
# ------------------------------------------------------------------
# TAB 2: GESTIÓN (RMS) - VINCULADO A LA API POST
# ------------------------------------------------------------------
with tab_gestion:
    ...
    if tipo_gestion == "Nuevo Agresor":
        with st.form("f_agr"):
            ...
            if st.form_submit_button("GUARDAR"): 
                payload = {"id_agresor": id_gen, "nombre_agresor": n, "apellido_agresor": a}
                resp = requests.post(f"{API_BASE_URL}/api/policia/nuevo_agresor", json=payload)
                if resp.status_code == 201: st.success(f"Registrado con éxito: {id_gen}"); st.rerun()
        ...
        with st.form("f_vic"):
            ...
            a = st.text_input("Apellidos")
            if st.form_submit_button("GUARDAR"): 
                payload = {"id_victima": id_gen, "nombre_victima": n, "apellido_victima": a}
                resp = requests.post(f"{API_BASE_URL}/api/policia/nueva_victima", json=payload)
                if resp.status_code == 201: st.success(f"Registrada con éxito: {id_gen}"); st.rerun()
                else: st.error("Error al registrar.")
        ...
        with st.form("f_sp"):
            ...
            if st.form_submit_button("GUARDAR"): 
                ...
                if resp.status_code == 201: st.success(f"Lugar registrado: {id_gen}"); st.rerun()
                else: st.error("Error al registrar.")
        ...
        with st.form("f_vinc_agr"):
            if not df_agresores.empty and not df_victimas.empty:
                ...
                if st.form_submit_button("VINCULAR ORDEN"):
                    ...
                    if resp.status_code == 201: st.success("Orden de alejamiento registrada."); st.rerun()
                    else: st.error("Error al vincular.")
            else:
        ...
        with st.form("f_vinc_sp"):
            if not df_victimas.empty and not df_safe_places.empty:
                ...
                if st.form_submit_button("ESTABLECER PERÍMETRO"):
                    ...
                    if resp.status_code == 201: st.success("Perímetro de seguridad asignado correctamente"); st.rerun()
                    else: st.error("Error al asignar perímetro.")
            else:
                st.warning("Faltan datos de víctimas o safe places.")

# ------------------------------------------------------------------
# TAB 3: BASE DE DATOS (VIA API Y BIGQUERY)
# ------------------------------------------------------------------
with tab_bbdd:
    st.markdown("### 📊 DIRECTORIO DE ÓRDENES DE ALEJAMIENTO (BIGQUERY)")
    if not df_relaciones.empty:
        st.dataframe(df_relaciones, use_container_width=True, hide_index=True)
    else:
        st.info("No hay relaciones de alejamiento activas.")
        
    st.divider()
    
    st.markdown("### 📍 CATÁLOGO DE LUGARES SEGUROS (BIGQUERY)")
    if not df_safe_places.empty:
        st.dataframe(df_safe_places, use_container_width=True, hide_index=True)
    else:
        st.info("No se han encontrado registros en la tabla de lugares seguros.")